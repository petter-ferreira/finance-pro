...
router.post('/', (req, res) => {
    ...
    db.query('SELECT id FROM customers WHERE id = $1 AND user_id = $2', [customer_id, userId])
        .then(result => {
            ...
            return db.query(sql, [customer_id, amount, amount, interest_rate])
                .then(loanResult => {
                    res.json({ message: 'Loan created', id: loanResult.rows[0].id });
                });
        })
        .catch(err => res.status(500).json({ error: err.message }));
});
...
router.post('/:id/update-interest', (req, res) => {
    ...
    db.query(sqlVerify, [loanId, userId])
        .then(result => {
            ...
            if (diffDays > 0) {
                ...
                return db.query(`UPDATE loans SET current_balance = $1, last_interest_update = $2 WHERE id = $3`,
